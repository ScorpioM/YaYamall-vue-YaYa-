    ...
    <div class="container">
        <div class="right_box">
          <!-- banner -->
            ...
            <el-breadcrumb-item>器械专区</el-breadcrumb-item>
            </el-breadcrumb>
            </div>
            <!-- contents -->
            <div class="top">
            ...
            <li v-for="(item, i) in newsList" :key="i">
              ...
              <div class="speacil" v-if="item.zhelv!==0">
                  特价!
              </div>
              <div class="move">
          ...
          <ul class="vertical" v-if="num == 1">
              <li v-for="(item, i) in newsList" :key="i" @click="jump(item)">
                  <div class="pic">
                       <img :src="item.addr[0]" alt="" />
          ...
          <el-radio-group v-model="radio" @change="categoryChange">
            <div v-for="(item, index) in tab" :key="index" class="radio">
              <el-radio :label="item.id"
                >{{ item.name }} <span class="num">({{item.num}})</span></el-radio
              >
            </div>
          </el-radio-group>
        </div>
  ...
  .container {
    ...
    .right_box {
      ...
      >.banner{
          width: 871px;
          height: 350px;
          background-image: url('../assets/img/instrument/bg.png');
  ...
  data() {
    return {
      ...
      newsList: [
        ...
        {
          pic: require("../assets/img/instrument/3.jpg"),
          name: "马来酸曲美布汀胶囊",
          price: 25,
          info:'Lorem ipsum dolor坐下来，自私自利。Fusce posuere metus vitae，imperdiet arcu，id aliquet前面的污染物。塞德（Sed）坐着，坐在生命中。',
        },
        {
          pic: require("../assets/img/instrument/2.jpg"),
          name: "马来酸曲美布汀胶囊",
          price: 25,
          info:'Lorem ipsum dolor坐下来，自私自利。Fusce posuere metus vitae，imperdiet arcu，id aliquet前面的污染物。塞德（Sed）坐着，坐在生命中。',
        },
  ...
  methods: {
    ...
    addCart(item) {
      this.$http
        ...
        // 商品分类
        getCategory(){
      this.$http.get('mt/qixie/findTypeAll',{
        params:{id:0}
      }).then(res=>{
    ...
    getProduct(){
            this.$http.get('mt/qixie/findAll',{
